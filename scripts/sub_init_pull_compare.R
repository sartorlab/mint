...
if(bool_pull_comp) {
	...
	for(i in 1:nrow(pulldown_comparisons)) {
		...
		make_rule_pull_compare = c(
			...
			sprintf('	$(PATH_TO_R) ../../scripts/annotatr_annotations.R --file $< --genome $(GENOME) --annot_type PePr --group1 $(CHIP1_NAME_%s) --group2 $(CHIP2_NAME_%s)', i, i),
			'',
			...
			sprintf('	$(PATH_TO_R) ../../scripts/annotatr_annotations.R --file $< --genome $(GENOME) --annot_type simple_pulldown_PePr --group1 $(CHIP1_NAME_%s) --group2 $(CHIP2_NAME_%s)', i, i),
			'',
