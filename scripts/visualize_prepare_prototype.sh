...
# Average change in methylation in DMRs in annotated regions
  ...
  bedtools intersect -a ../data/annotation/cpg_islands_hg19_ucsc.bed -b <(awk -v OFS='\t' 'NR > 1 && $5 < 0.05 { print $0 }' ./analysis/methylsig_calls/IDH2mut_v_NBM.txt) -wa -wb | bedtools groupby -g 1-3 -c 10 -o mean > IDH2mut_v_NBM_mc_hmc_avg_meth_chg_in_DM_cpg_islands.bed

  # Find average % methylation change in CpG islands
  bedtools intersect -a ../data/annotation/cpg_shores_hg19_ucsc.bed -b <(awk -v OFS='\t' 'NR > 1 && $5 < 0.05 { print $0 }' ./analysis/methylsig_calls/IDH2mut_v_NBM.txt) -wa -wb | bedtools groupby -g 1-3 -c 10 -o mean > IDH2mut_v_NBM_mc_hmc_avg_meth_chg_in_DM_cpg_shores.bed

  # Find average % methylation change in promoters
  bedtools intersect -a ../data/annotation/ldef_5kb_hg19_reduced.bed -b <(awk -v OFS='\t' 'NR > 1 && $5 < 0.05 { print $0 }' ./analysis/methylsig_calls/IDH2mut_v_NBM.txt) -wa -wb | bedtools groupby -g 1-4 -c 11 -o mean | awk -v OFS='\t' '{print $1, $2, $3, $5, $4}' > IDH2mut_v_NBM_mc_hmc_avg_meth_chg_in_DM_promoters.bed

  # Find average % methylation change in enhancers
  bedtools intersect -a ../data/annotation/andersson_permissive_enhancers.bed -b <(awk -v OFS='\t' 'NR > 1 && $5 < 0.05 { print $0 }' ./analysis/methylsig_calls/IDH2mut_v_NBM.txt) -wa -wb | bedtools groupby -g 1-3 -c 19 -o mean > IDH2mut_v_NBM_mc_hmc_avg_meth_chg_in_DM_andersson_permissive_enhanceres.bed
